  ...
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  public updateView(
    ...
    const undoButtonText =
      context.parameters.UndoButtonText.raw === 'val'
        ? 'Undo'
        : context.parameters.UndoButtonText.raw || 'Undo';
    const verifyButtonText =
      context.parameters.VerifyButtonText.raw === 'val'
        ? 'Check Answers'
        : context.parameters.VerifyButtonText.raw || 'Check Answers';
    const incorrectMessage =
      context.parameters.IncorrectMessage.raw === 'val'
        ? 'Incorrect'
        : context.parameters.IncorrectMessage.raw || 'Incorrect';
    const congratulationsMessage =
      context.parameters.CongratulationsMessage.raw === 'val'
        ? 'Congratulations'
        : context.parameters.CongratulationsMessage.raw || 'Congratulations';
    ...
    if (elementsListValue !== this._previouselementsListValue) {
      ...
      try {
        ...
        if (
          !Array.isArray(this._elementsListArray) ||
          !this._elementsListArray.every(
    ...
    rules.forEach((rule) => {
      ...
      try {
        if (this._customStyleElement && this._customStyleElement.sheet) {
          (this._customStyleElement.sheet as CSSStyleSheet).insertRule(
            rule,
            this._customStyleElement.sheet.cssRules.length,
          );
          // console.log(this._customStyleElement)
        } else {
          if (this._customStyleElement) {
  ...
  public getOutputs(): IOutputs {
    return {
      nbWrongAnswers: `${this._nbWrongAnswers}`,
      allAnswersCorrect: `${this._allAnswersCorrect}`,
    };
  }
