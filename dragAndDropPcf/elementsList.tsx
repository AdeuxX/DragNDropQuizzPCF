...
export class ElementsList {
    ...
    constructor(elements: string[][]) {
        this._elements = elements.map(pair => pair.map(item => this.parseElement(item)));
        this._elementsLeft = this._elements.map(e => e[0]).sort(() => Math.random() - 0.5);
        this._elementsRight = this._elements.map(e => e[1]).sort(() => Math.random() - 0.5);
    }

    private parseElement(item: string): { type: 'text' | 'image', value: string } {
    ...
    public checkIfCorrectAssociation(elementsTracked: { elementInit: string, elementFinal: string }): boolean {
        for (const coupleElement of this._elements) {
            if (coupleElement.some(e => e.value === elementsTracked.elementInit) &&
    ...
    public checkIfElementsSameSide(elementsTracked: { elementInit: string, elementFinal: string }): boolean {
        const leftElements = this.getElements("left");
        const rightElements = this.getElements("right");
        return (leftElements.some(e => e.value === elementsTracked.elementInit) && leftElements.some(e => e.value === elementsTracked.elementFinal)) ||
               (rightElements.some(e => e.value === elementsTracked.elementInit) && rightElements.some(e => e.value === elementsTracked.elementFinal));
    }

    public setNewOrdre() { }

    public getElements(side: "left" | "right"): { type: 'text' | 'image', value: string }[] {
